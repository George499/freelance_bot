...
async def get_kwork_projects(bot: Bot, config: Settings):
    ...
    for project in projects:
        # Kwork отдаёт в API-ленте заказы со status="new" — они ВНЕ пула
        # видимости нашего аккаунта: и /projects/{id}, и /projects/{id}/view
        # редиректят на общую биржу, и в самой бирже заказа нет. Откликнуться
        # нельзя. Скорее всего это ранний доступ по уровню/рейтингу (проверенные
        # видят заказ раньше — деньгами купить нельзя), новичку он недоступен.
        # Проверено на аккаунте: 3221314(new) висит new >суток с 11 откликами,
        # но у нас редиректит и в биржу не попадает; active-заказы открываются.
        # Показываем ТОЛЬКО "active". "new" пропускаем без записи в БД (мелькнёт
        # active — поймаем; их 0-1 на ленту, холостой прогон ничтожен).
