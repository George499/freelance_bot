...
_STACK_OK = re.compile(
    ...
    # Правка 25.09. AI-агенты и LLM - основная ниша George, но заказчики
    # часто пишут без слова «бот»: «Перестроить AI-агента в TG: router,
    # state, промпт» (25.09) и «ИИ ассистент... через telegram, ответы через
    # LLM» (24.09) ушли с «нет маркеров моего стека». Берём только термины
    # разработки: голые «нейросеть», «промпт», «ChatGPT» тянут контентные
    # заказы (картинки, тексты), их не добавляем.
    r"\bllm\b|\bopenai\b|\brag\b|system\s*prompt|"
    r"\bии[\s-]?(?:ассистент|агент)\w*|\bai[\s-]?(?:агент|agent|ассистент|assistant)\w*|"
    r"mini[\s-]?app|мини[\s-]?прил\w+|vk\s*api|long\s*poll|"
    # «парс(ер|инг|и)» - ещё и «распарсить», «спарсить»: 25.09 так ушёл
    # «Извлечь статьи из бэкапа blogspot» («либо распарсить наш блог»).
    r"\bapi\b|интеграц\w+|вебхук\w*|webhook|\bparser\b|парс(?:ер|инг|и)\w*|"
    # Правка 24.09. Заказчики сплошь пишут на бизнес-языке, без слова
    # «парсер». 23.09 потерян «Поиск аукционов по ключам на сайте
    # госзакупок»: «в поиске ЕИС по имеющему токену получать список
    # тендеров... по набору слов и минус слов, по ИНН и периоду» — это
    # чистый Python + API, но ни одного маркера из белого списка в тексте.
    r"\bеис\b|госзакуп\w*|тендер\w*|zakupki|\bтокен\w*|"
    # Та же правка, другая сторона: лучшие заказы вообще не содержат
    # технических слов. «Нужна разработка кастомная под ключ. Стек
    # предлагайте» — это идеальный заказ (бюджет не сбит, конкуренции по
    # ключевику нет), а белый список его не узнавал и он ушёл 23.09.
    # Безопасно, потому что чёрный список ниже сильнее: WordPress, Тильда,
    # Битрикс, мобилки и вёрстка отсекаются даже при слове «разработка».
    r"разработ\w+\s+(?:сайт\w*|веб|платформ\w+|сервис\w*|систем\w+|приложен\w+)|"
    r"личн\w+\s+кабинет\w*|\bплатформ\w+|стек\s+предлаг\w+|"
...
_STACK_BAD = re.compile(
    ...
    r"консультац\w+|консультант\w*|репетитор\w*|"
    # Правка 24.09. Было просто «обучени\w+» — и 23.09 оно зарубило
    # «Разработка сайта и личного кабинета. Стек предлагайте» только потому,
    # что проект назывался «Цифровая платформа обучения и аттестации
    # сотрудников». Слово «обучение» здесь ДОМЕН, а не заказанная услуга —
    # ровно та же ошибка, что раньше с CMS. Ловим только просьбу научить.
    r"обучени[еяю]\s+(?:python|питону|программирован\w+|разработк\w+)|"
    r"обучить?\s+(?:меня|нас|сотрудник\w*|команду|персонал)|"
    r"провести\s+обучени|нужен\s+(?:преподавател\w*|наставник\w*|ментор\w*)|"
