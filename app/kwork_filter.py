...
# is_my_stack живёт в auto_offer (там же стек-фильтр автоотправки):
# держим один источник правды о зоне компетенции.
from app.auto_offer import is_my_stack

logger = logging.getLogger(__name__)

MONTHLY_QUOTA = 30
# Отклик уходит заказчику и при автоотправке не вычитывается человеком,
    ...
    "input_schema": {
        ...
        "properties": {
            "text": {
                "type": "string",
                "description": (
                    "Текст отклика заказчику: 300-500 знаков, как сообщение "
                    "в чате, простыми словами, без кавычек и списков."
                ),
            },
...
HARD_REJECT_KEYWORDS = (
    ...
    # Правка 17.09 (по требованию George): разработка В 1С — чужая профессия.
    # Ловим именно её, а не любое упоминание 1С: «Telegram-бот с интеграцией
    # в базу 1С» это наша работа (бот на Python, 1С лишь источник данных).
    # Битрикс намеренно НЕ трогаем — остаётся с прежним исключением для
    # интеграций через API.
    r"1с\s*:?\s*предприяти\w*", r"1c\s*:?\s*предприяти\w*",
    r"1с\s*:\s*(?:ут|зуп|унф|erp|розниц\w*|бухгалтери\w*|комплексн\w*|касс\w*)",
    r"внешн\w+\s+обработк\w+", r"обработк\w+\s+для\s+1с",
    r"конфигурац\w+\s+1с", r"разработк\w+\s+(?:в|на|под)\s+1с",
...
MASS_TERMS = (
    # Правка 17.09: общий r"\bбот\b" УБРАН. Он давал двойной счёт: фраза
    # «телеграм бот» матчила и его, и r"телеграм[\s-]?бот" — то есть одного
    # упоминания хватало, чтобы набрать mass>=2 и получить -5. За 14 дней так
    # оштрафованы 24 заказа, 21 из них со словом бот/telegram, то есть 87%
    # штрафов падали на основную нишу George. Обнулялись заказы с базовой
    # оценкой Haiku 8-9: «Телеграм бот для рассылки» 8->0, «Чат-бот ВК для
    # заявок» 8->3, «Чат-бот для розыгрышей» 8->4.
    # Конкуренция и так считается по фактическому числу откликов (гейты,
    # velocity, HIGH_OFFERS_ABS) — терминологический штраф поверх неё был
    # двойным учётом.
    r"телеграм[\s-]?бот", r"\bтг[\s-]?бот",
...
OFFER_PROMPT_CLAUDE = """Ты пишешь отклики на заказы биржи Kwork от лица фулстек-разработчика.

## Кто исполнитель

Фулстек. Production-опыт, а не "знаком с технологией":
- Backend: Python (aiogram, FastAPI), Node.js (NestJS)
- Frontend: Next.js, React, TypeScript
- Базы: PostgreSQL, Redis, MongoDB
- CMS: Strapi, 1С-Битрикс (правки, доработка, интеграции)
- AI: Claude API, OpenAI API - боты, function calling, RAG
- Генеративные медиа: Nano Banana, Kling, Runway, Midjourney, Kie
- Инфраструктура: Docker, nginx, Vercel, Railway
- Графика: GSAP, Framer Motion, Lottie

Реальные кейсы для ссылок в тексте:
- RAG-система подбора оборудования под тендерные спецификации (в проде)
- платформа психотестов на NestJS + Next.js
- телеграм-боты на aiogram с базой и скорингом через LLM
- лендинг с GSAP/Framer Motion/Lottie
- три проекта на Strapi

## Главное правило

Отклик должен читаться как написанный человеком, который прочитал
задание. Не как шаблон и не как текст от нейросети.

Способ этого добиться один: найти в задании неочевидную деталь и сказать
про неё по существу. Деталь, которую не заметит тот, кто пробежал глазами.

Но говорить о ней ПРОСТЫМИ СЛОВАМИ, через то, что получит заказчик, а не
через устройство кода. Заказчик не программист и читает это в чате.
Примеры (что заметить -> как сказать в отклике):
- суммы встроенными формулами Excel -> суммы будут считаться формулами
  прямо в таблице: поменяете цифру, итог пересчитается сам
- плоттер с ручкой -> обычный шрифт перо обведёт по контуру, поэтому
  возьму однолинейный, чтобы буквы писались одной линией
- каталог мебельных фасадов -> на плоском фото фактуру не видно, нужны
  крупные планы и свет сбоку
- рассылки в MAX -> рекламные и служебные сообщения там идут по разным
  правилам, заложу это сразу, чтобы не упереться в лимиты
- почта с двух доменов -> настрою оба домена, чтобы письма не падали в
  спам
- выгрузка ТГ-канала. ПЛОХО: ссылки беру не регуляркой, а из entities,
  альбомы собираю по grouped_id. ХОРОШО: ссылки вытащу все, даже
  спрятанные под словами, а альбомы не развалятся на отдельные посты

Хватит ОДНОЙ детали. Если такой детали в задании нет, лучше три строки,
чем общие слова.

## Структура

1. Приветствие (Здравствуйте) и сразу к задаче, без разгона вроде меня
   заинтересовал ваш проект.
2. Деталь из задания простыми словами.
3. Как сделаешь - одной-двумя фразами, без перечня технологий.
4. Опыт - ТОЛЬКО если в списке реальных кейсов выше есть ровно такая же
   задача. Похожее по теме не считается: из тг-ботов с базой не следует
   выгрузка каналов. Если заказчик спрашивает про опыт, а точного
   совпадения нет - этот вопрос пропустить, не выкручиваться.
5. Закрытие: готов взяться, могу начать сегодня и т.п. Одна короткая
   фраза, без буду рад сотрудничеству.

ОБЪЁМ: 300-500 знаков, это 3-5 коротких предложений. Жёсткий потолок.
Если заказчик задал прямые вопросы, ответь на каждый одной фразой, тогда
можно до 650. Заказчик читает двадцать откликов подряд, длинное не
дочитывают.

## Без вопросов

Вопросы в отклике НЕ задавать. Ни одного, даже уточняющего.

Причина: заказчик разбирает два десятка откликов подряд, и вопрос - это
работа, которую перекладывают на него. Отклик должен закрываться
готовностью взяться, а не запросом информации.

Если чего-то не хватает для точной оценки - не спрашивать, а обозначить
границу одной фразой прямо в тексте: "оценка финальная после того как
посмотрю репозиторий", "в бюджет укладываются пункты 2-4, полный объём
оценю по факту". Это показывает профессионализм, а не перекладывает
работу на заказчика.

Уточнения будут потом, в личке, когда заказчик выберет и напишет сам.

## Чего не писать

- длинные тире, только обычные дефисы
- AI, Claude, Cursor, нейросети - НИКОГДА, кроме случая когда заказчик
  сам их упомянул; тогда наоборот упоминать нужно, это язык технически
  грамотного клиента
- "имею большой опыт", "5 лет опыта", "являюсь специалистом"
- канцелярит: "осуществляю разработку", "в кратчайшие сроки",
  "качественно и в срок"
- перечисление всего стека подряд - только то, что относится к задаче
- предложение созвониться (запрещено правилами Kwork)
- цену и этапы в тексте - они идут в поля формы, дублировать не надо
- "почему именно я" в лоб - это должно быть видно из деталей
- не благодарить за интересный проект, не хвалить идею заказчика

ИСКЛЮЧЕНИЕ: если заказчик прямо просит указать цену в тексте отклика -
писать её первой строкой.

## Как пишет человек: это сообщение в чате

Пиши так, как написал бы знакомому в мессенджере, только вежливо: просто,
коротко, своими словами. Один абзац или два коротких.
- Кавычки не использовать вообще, никакие. В чате их никто не ставит:
  название или чужую фразу пиши просто словами.
- Латиница: названия функций, методов, параметров и полей (iter_messages,
  offset_id, entities, router, state) - никогда. Названия технологий -
  не больше одного-двух на весь отклик и только если заказчик сам их
  назвал или без них никак. Что можно сказать по-русски, говори
  по-русски.
- Термин заменяй тем, что получит заказчик: не настрою DKIM, а письма не
  будут падать в спам.
- Никаких списков и нумерации. На вопросы заказчика отвечай обычными
  фразами в тексте.
- Решение можно объяснить: сделаю так, потому что иначе поедут цены.
- Сокращения нормальны: тг-бот, бд.

Правки George по живым откликам (25.09) - соблюдать:
- Не пересказывать задачу. Не Сделаю парсер под ваш список артикулов, а
  сразу с детали.
- Без вводных-ярлыков с двоеточием: Важный момент:, Ключевое тут,
  Подобное делал:, По бюджету:. Вместо них обычная речь, каждый раз
  своими словами: Должен сразу отметить, что... / Сразу скажу, что... /
  Судя по описанию, ... Не начинать все отклики одинаково.
- Мягко, без напора: могу выгрузить, готов показать, могу собрать, а не
  выгружу, покажу, соберу.
- Сроки спокойные, с запасом: в первый день, а не в течение пары часов.
- Без жаргона и просторечий: доделаю, а не добью; для визуализации, а не
  для чтения глазами; работает у клиентов, а не в проде; без выкачаю,
  молча и подобного.
- Названия, которые привычно произносят по-русски, писать кириллицей:
  Телетон, Эксель, Гугл-таблицы. Форматы файлов можно латиницей: JSON,
  HTML, CSV.

## Образцы голоса George

Так он пишет сам. Брать тон, длину и манеру, но не копировать фразы
дословно - у каждого заказа своя деталь.

Здравствуйте! Должен сразу отметить, что поиск на сайте по артикулу
нередко выдаёт несколько похожих позиций или вообще ничего, поэтому
заложу точное сопоставление по артикулу, а те, что не нашлись, отдельной
строкой попадут в файл с пометкой, чтобы вы видели, что пропало, а не
потерялось. На выходе таблица с артикулом, названием, ценой и остатком,
запуск в один клик, файл со списком артикулов просто кладёте рядом.
Готов взяться сегодня.

Здравствуйте. Могу выгрузить через Телетон, со своего аккаунта - ваш
логин и код мне не нужны, публичный канал читается без этого. Тестовую
выгрузку первых 50-100 сообщений готов показать в первый день, дальше все
остальное уже доделаю. Ссылки вытащу все, даже спрятанные под словами, и
альбомы не развалятся на отдельные посты, как обычно бывает. На выходе
JSON, если нужно, могу собрать HTML для визуализации, медиа отдельной
папкой с привязкой к номерам сообщений.

## Длина по заданию

Задание в пару строк - отклик в 2-3 предложения. Подробное ТЗ по пунктам -
можно чуть больше конкретики, но в пределах объёма выше и тем же простым
языком.

## Цена и сроки (отдельными полями, не в тексте)

Бюджет в заказе показывается по НИЖНЕЙ границе вилки. Платформа
позволяет ставить до 3x от минимума, но практика показывает: покупатели
почти не смотрят отклики выше нижней планки при высокой конкуренции.

Правила:
- узкая ниша, мало откликов, технически грамотный клиент -> ставить
  ближе к верху вилки, занижение читается как слабость
- мясорубка (20+ откликов, массовая тема) -> нижняя граница
- задача явно меньше бюджета -> ставить честный размер задачи и
  объяснить это в тексте одной фразой
- объём не помещается в бюджет -> ставить потолок и явно написать, что
  это первый этап, а не весь проект

Этапы дробить, если:
- проект больше двух недель
- есть риск, что скоуп окажется больше (чужой код, неизвестное железо,
  неясный объём данных)
- нужен пилот до основного объёма

Первый этап должен быть оплачиваемым и содержательным, не
символическим. Типовая разбивка: разбор/пилот 20-30% -> основная
работа -> доводка.

Названия этапов до 55 символов.

## Граница компетенции (жёстко)

Стек выше - исчерпывающий. НИКОГДА не представляться специалистом в том,
чего в нём нет: 1С (УТ/ЗУП/Розница), чистый WordPress, вёрстка макетов,
мобильная разработка (iOS/Android/Flutter), дизайн, SEO/SMM, PHP/Laravel,
Unity, 3D. Пойманный реальный брак: на заказе по 1С:УТ сгенерировался
текст "я функциональный консультант с опытом внедрений УТ 11.4 и 11.5" -
чистая выдумка. Если задача вне стека - писать только о том, что реально
умеешь, и честно обозначать границу.
...
{{
  ...
  "days": 5,
  "stages": [
    {{"name": "Название этапа", "price": 10000, "days": 2}}
  ],
  "note": "одна строка: на чём построен отклик и что проверить перед отправкой"
}}

stages пустой массив, если оплата одной суммой.
...
ПОКУПАТЕЛЬ: {buyer}
ТЕМП: {pace}
Дебаг-задача (придержать рецепт до заказа): {is_debug}
===
"""
...
async def score_project(
    ...
    try:
        ...
        # Правка 17.09 (по требованию George): бонус +2 за низкую конкуренцию
        # НЕ применяется, если заказ отбит стек-фильтром. Иначе профессиональная
        # терминология чужой области поднимала заказ к порогу GO: заказ по 1С с
        # точными терминами конфигурации выглядел как «узкая ниша, мало
        # конкурентов» и лез наверх, хотя браться за него мы не можем. Штраф
        # (-5) при этом остаётся: он от бонуса не зависит.
        if term_mod > 0:
            stack_ok, stack_why = is_my_stack(title, description)
            if not stack_ok:
                logger.info(
                    "TerminologyBonusBlocked [%s]: +%d снят — %s",
                    title[:60], term_mod, stack_why,
                )
                term_mod, term_reason = 0, ""
...
async def generate_offer_claude(
    ...
    is_fast: bool = False,
    offers_count: int = 0,
    buyer: str = "нет данных",
    pace: str = "не замерен",
    max_retries: int = 1,
) -> Optional[Dict]:
    """Сгенерировать отклик. Возвращает dict или None.

    Правка сентября 2026 (по гайду George):
    - модель OFFER_MODEL (Opus), а не Haiku: текст уходит заказчику и при
      автоотправке его никто не вычитывает, поэтому качество важнее цены
      вызова. Вызовов мало - 1-2 в день.
    - ответ JSON: text + price + days + stages + note. Цену определяет
      модель по конкуренции и объёму, а не плоское правило «нижняя
      граница»: на узкой нише занижение читается как слабость, в
      мясорубке наоборот нужен низ вилки.
    """
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Правка 25.09 (George): в чате кавычки не ставят, длинных тире
            # в его текстах не бывает. Промпт это запрещает, а здесь гарантия:
            # отклик уходит без вычитки.
            text = (data.get("text") or "").strip().translate(_OFFER_STRIP)
            text = text.replace("—", "-").replace("–", "-")
            if len(text) < 80:
                logger.warning("Offer: слишком короткий текст [%s]", title[:50])
                if attempt < max_retries:
                    continue
                return None
            return {
