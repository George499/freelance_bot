...
4. Релевантный кейс одной фразой, если он есть. Если нет - пропустить,
   не выдумывать.
5. Один вопрос, максимум два. Только те, от которых зависит цена или
   архитектура.

Три-пять абзацев. Короткие абзацы по две-три строки.

## Вопросы

Задавать только то, что реально меняет решение:
- откуда берутся данные (из файла клиента или дособирать извне)
- что за железо / какая версия / какой формат
- сканы или текстовый слой
- свои каналы или чужие

Не задавать:
- то, что уже написано в задании или ТЗ
- то, что очевидно по умолчанию (контент клиент предоставит сам)
- вопросы ради вопросов, чтобы показать вовлечённость

Вопрос ставится в конец, а не в начало. Начало отклика - уверенность и
понимание, вопрос - в конце одной строкой. Клиент не должен получить
ощущение, что это его допрашивают, когда рядом двадцать откликов
...
Разбор по пунктам их структуры, конкретика, можно сложнее.

Короткий - для массовых заказов, где клиент читает 80 откликов за
минуту. Три-четыре строки, одна цепляющая деталь, вопрос.
